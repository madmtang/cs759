...
def _stage1_lse_kernel(
    ...
    for k0 in range(0, 2048, BLOCK_K):
        ...
        for d0 in range(0, 64, BLOCK_DKPE):
            offs_d = d0 + tl.arange(0, BLOCK_DKPE)
            q_ptrs = q_pe_ptr + pid_t * stride_qpet + pid_h * stride_qpeh + offs_d * stride_qped
            q = tl.load(q_ptrs, mask=offs_d < 64, other=0.0).to(tl.float32)
            ...
            k = tl.load(
                k_ptrs,
                mask=valid[:, None] & (offs_d[None, :] < 64),
                other=0.0,
            ).to(tl.float32)
            logits += tl.sum(k * q[None, :], axis=1)
...
def run(*args, **kwargs):
    ...
    q_nope, orig_devices["q_nope"] = _to_cuda_if_needed(q_nope, "q_nope")
    q_pe, orig_devices["q_pe"] = _to_cuda_if_needed(q_pe, "q_pe")
    ckv_cache, orig_devices["ckv_cache"] = _to_cuda_if_needed(ckv_cache, "ckv_cache")
    kpe_cache, orig_devices["kpe_cache"] = _to_cuda_if_needed(kpe_cache, "kpe_cache")
    sparse_indices, orig_devices["sparse_indices"] = _to_cuda_if_needed(sparse_indices, "sparse_indices")

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Triton execution.")

    if torch.is_tensor(sm_scale):
        sm_scale, orig_devices["sm_scale"] = _to_cuda_if_needed(sm_scale, "sm_scale")
